...
import {getAppToken,getUserToken,refreshUserToken} from 'src/helper/feishu/auth'
...
export class FeishuService {
  ...
  constructor( 
    ...
    this.USER_TOKEN_CACHE_KEY=this.configService.get('FEISHU_CONFIG')['USER_TOKEN_CACHE_KEY']
    this.USER_REFRESH_TOKEN_CACHE_KEY=this.configService.get('FEISHU_CONFIG')['USER_REFRESH_TOKEN_CACHE_KEY']
  }
...
async setUserCacheToken(tokenInfo: any) {
  ...
  //   ttl: expires_in - 60,
  // });
  await client.set(`${this.USER_TOKEN_CACHE_KEY}_${user_id}`,access_token)
  // await client.expire(`${this.USER_TOKEN_CACHE_KEY}_${user_id}`,expires_in - 60)
  await client.expire(`${this.USER_TOKEN_CACHE_KEY}_${user_id}`,1)
  ...
  //   },
  // );
  await client.set(`${this.USER_REFRESH_TOKEN_CACHE_KEY}_${user_id}`,refresh_token)
  await client.expire(`${this.USER_REFRESH_TOKEN_CACHE_KEY}_${user_id}`,refresh_expires_in - 60)
}
...
async getCachedUserToken(userId: string) {
  ...
  // );
  let userToken: string = await client.get(`${this.USER_TOKEN_CACHE_KEY}_${userId}`)
  ...
  if (!userToken) {
    ...
    const refreshToken: string = await client.get(
      `${this.USER_REFRESH_TOKEN_CACHE_KEY}_${userId}`,
    );
    if (!refreshToken) {
